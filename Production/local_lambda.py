...
def euler_to_Rot(yaw, pitch, roll):
    ...
    logging.debug(f'yaw: {yaw}, roll: {roll}, pitch: {pitch}, Rot: {Rot}')
...
def extract_coords(prediction, flipped=False):
    ...
    logging.debug(f'predicted coords" {coords}')
    return coords
...
model = torch.load('final_model.pt', map_location=torch.device('cpu'))
...
def main(argv):
    ...
    try:
        ...
        print('lambda_local.py -i <inputfile> -o <outputfile>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('lambda_local.py -i <inputfile> -o <outputfile>')
    ...
    logging.info(f'Output file is {outputfile}')

    print('Output file is "', outputfile)

    img0 = imread(inputfile, True)
    img = preprocess_image(img0, flip=flip)
    plt.imshow(img)
    img = np.rollaxis(img, 2, 0)
    ...
    with open('coords_pred.npy', 'wb') as f:
        np.save(f, coords_pred)
        
    plt.clf()

    img0 = imread(inputfile)
